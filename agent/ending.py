...
from .utils import generate_possible_moves, place_tetromino, render_board, count_holes
...
class Ending:
    ...
    def generate_ending_move(self) -> PlaceAction:
        my_possible_moves = generate_possible_moves(self.board, self.mycolor)
        ...
        for move in my_possible_moves:
            ...
            oppo_possible_moves = generate_possible_moves(new_board, oppo_color)
            if len(oppo_possible_moves) == 0:
                return move  # Found a move where opponent has no possible moves
            if len(oppo_possible_moves) < min_oppo_possible_moves or (len(oppo_possible_moves) == min_oppo_possible_moves and count_holes(new_board) % 2 == 0):
                min_move = move
